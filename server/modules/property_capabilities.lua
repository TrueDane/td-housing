...
local function notifyError(playerSource, errorCode)
    ...
    TD.Notify(playerSource, {
        title = 'TD-Housing',
        description = messages[errorCode] or ('Handlingen kunne ikke gennemføres (%s).'):format(errorCode or 'UNKNOWN_ERROR'),
        type = 'error',
    })
end
...
RegisterNetEvent('td_housing:server:buyFurniture', function(propertyId, items, isGarden)
    ...
    TD.Notify(playerSource, {
        title = 'TD-Housing',
        description = result.totalPrice > 0 and ('Møbler købt for $%s.'):format(result.totalPrice) or 'Boligfunktionen er placeret.',
        type = 'success',
    })
end)
