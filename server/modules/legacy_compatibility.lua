...
RegisterNetEvent("ps-housing:server:updateFurniture", function(propertyId, item)
	...
	local success, errorCode =
		PropertyCapabilityService.UpdateFurniture(playerSource, propertyId, item.id, item.position, item.rotation)
...
lib.callback.register("ps-housing:cb:inventoryHasItems", function(source, stashId)
	...
	if
		not property
		or not PropertyCapabilities.CanManageFeatures(property.propertyData, TD.Player.GetIdentifier(source))
	then
		return true
	end
