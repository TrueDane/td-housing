...
local function registerStorage(property, furniture)
    ...
    local label = property.propertyData.street or property.propertyData.apartment or ('Property #%s'):format(property.property_id)
...
local function syncCapabilities(property)
    ...
    if updated then
        ...
        TriggerClientEvent('ps-housing:client:updateProperty', -1, 'UpdateHas_access', property.property_id, legacyAccess)
    end
...
Property.PlayerEnter = function(self, playerSource)
    ...
    if not PropertyCapabilities.HasDoorAccess(self.propertyData, identifier) and not canEnterTemporarily(self, playerSource) then
        return false
    end
